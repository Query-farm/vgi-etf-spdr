...
// `bin/vgi-etf-spdr-worker` and talks to it over stdin/stdout; the haybarn suite exercises that).
...
//   - a full products scan round-trips over HTTP                    [live: SSGA]
//
// The final scan hits SSGA live (like the haybarn live-invariant asserts) — fine for an egress
...
import { makeProductsScan, makeHoldingsScan, makeNavHistoryFunction } from "../src/functions.js";
...
test("catalog is discoverable over HTTP", async () => {
  ...
  try {
    ...
    expect(await client.catalogs()).toContain("spdr");
  } finally {
    rpc.close();
  }
});
...
test("the functions and the products/holdings tables are exposed over HTTP", async () => {
  ...
  try {
    const client = new VgiClient(rpc);
    const attach = await client.catalogAttach("spdr");
    const fns = await client.schemaContentsFunctions(attach.attach_opaque_data, "main", "TABLE_FUNCTION");
    // The holdings scan is listed under the SAME name as its base table (required so the
    // extension pushes the ticker filter into the holdings table); products' backing scan stays
    // unlisted. So the listed table functions are `holdings` (= the holdings table's scan) and
    // `nav_history`.
    expect(fns.map((f) => f.name).sort()).toEqual(["holdings", "nav_history"]);
...
test("products table scan round-trips over HTTP (live)", async () => {
  ...
  try {
    ...
    // Live invariant: SPY is in the ETF list.
